...
public:
	...
	void cuela(const T& a, const T& b) {
		...
		if (numElems < 3) return;

		while (_elem != ult) {
			if (_pos->elem != a) _pos = _pos->sig;
			if (_aux->sig->elem != b) _aux = _aux->sig;
			_elem = _aux->sig;
			if (_elem->elem == b && _pos->elem == a) break;
		}
