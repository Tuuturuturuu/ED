...
private:
    ...
    void invert();
    void moveNumbersToOther(ListLinkedSingle& dest);
    void append(ListLinkedSingle& dest);
};
...
// O(N) puesto que last_node() es O(N) siendo N el numero de elementos de la lista this
void ListLinkedSingle::append(ListLinkedSingle& dest2) {
    if (this->head != nullptr) {
        Node* last = this->last_node();
        last->next = dest2.head;
        dest2.head = nullptr;
    }
    else {
        this->head = dest2.head;
        dest2.head = nullptr;
    }
}

//O(N) siendo N el numero de elementos de la lista dest o de this en el peor caso (aquella que sea mas larga)
void ListLinkedSingle::moveNumbersToOther(ListLinkedSingle& dest) {
    if (!isalpha(this->head->value)) {
        Node* curr = head;
        Node* other = dest.head;

        while (dest.head != nullptr && other->next != nullptr) {
            other = other->next;
        }

        while (!isalpha(curr->value)) {
            head = head->next;
            curr->next = nullptr;
...
/*
*
Este ejercicio/ metodo sería mucho mas sencillo trabajando con una lista
...
el coste final del metodo tambien lo será)

*/
void ListLinkedSingle::destripar(ListLinkedSingle& dest) {
    this->moveNumbersToOther(dest);//O(N)
    this->invert();//O(N)
    ListLinkedSingle dst2;
    this->moveNumbersToOther(dst2);
    this->invert();
    dst2.invert();
    dest.append(dst2);
}
