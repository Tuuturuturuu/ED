...
public:
    ...
    void nueva_serpiente(const string& nombre, const Posicion& posicion) {
        ...
        //marcar la posicoin del tablero introducida como ocupada por una serpiente
    ...
    void nueva_manzana(const Posicion& posicion, int crecimiento, int puntuacion) {
        ...
        //marcar la posicoin del tablero introducida como ocupada por una manzana
    ...
    //Complejidad: en el caso general, avanzar la serpiente tiene coste O(1) (constante), excepto en el 
    ...
    bool avanzar(const string& nombre, const Direccion& dir) {
        ...
        //hse aumentan las coords de la cabeza de la serpiente en la direccion introducida
        int x = infoS.pos.x;
        int y = infoS.pos.y;
        ...
        //comprobamos si la casilla destino a la que queremos avanzar la cabeza esta ocupada y 
        //que elemento ocupa
        if (que_hay({ x,y }) == Elemento::Serpiente) {//casilla ocupada por serpiente 
            ...
            if ((infoS.cuerpo.back().x == x) && (infoS.cuerpo.back().y == y) && infoS.crecimiento == 0) {//la casilla destino es la que ocupa la cola de la misma serpiente
                ...
                int longitud = mapaSerpientes[nombre].cuerpo.size();//O(1)
    ...
    while (instruc != "FIN") {
        try {
            if (instruc == "nueva_serpiente") {
                cin >> nom >> x >> y;
                juego.nueva_serpiente(nom, { x,y });//O(1)
            }
                ...
                cin >> nom >> dir;
                if (juego.avanzar(nom, dir)) {//O(1)/O(N)
                    cout << nom << " muere\n";
                }
            }
